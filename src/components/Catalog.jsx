...
// ── Materiales fijos — exactamente 4 opciones ─────────────────
const MATERIALS = [
  { key: 'all',          label: 'Todos'        },
...
// ── Cargar más: cuántos productos mostrar por vez ─────────────
const PAGE_SIZE = 20
...
export default function Catalog() {
  ...
  // Resetear visibleCount cuando cambian los filtros o la búsqueda
  ...
  // Solo los primeros N para renderizar
    ...
    <section id="catalogo" className="px-6 py-24 bg-white md:px-12">
      ...
      {!loading && !error && products.length > 0 && (
        <div className="flex flex-col gap-4 mb-12">
                ...
                <button key={c.key} onClick={() => handleCategory(c.key)}
                  className={`text-[11px] tracking-[0.1em] uppercase px-4 py-1.5 rounded-full border font-sans transition-all duration-200
                    ${activeCategory === c.key
                      ...
                      : 'bg-transparent text-[#7a7269] border-[#e8e2da] hover:border-[#0e0d0c] hover:text-[#0e0d0c]'}`}>
                  {c.label}
                </button>
              ))}
            </div>
          ...
          {/* Fila 3: Material — exactamente 4 opciones fijas */}
          <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="grid grid-cols-2 gap-5 md:grid-cols-3 lg:grid-cols-4 md:gap-7">
        ...
        {/* Botón "Ver más" dentro del grid para ocupar el ancho completo */}
