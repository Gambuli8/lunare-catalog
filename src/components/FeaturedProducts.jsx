...
function FeaturedImage({ image, emoji, name }) {
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-white to-[#ede7df] flex items-center justify-center">
      <span className="text-6xl select-none opacity-50 group-hover:scale-110 transition-transform duration-500">{emoji}</span>
    </div>
  )
}
    ...
    >
      <div className="relative aspect-square overflow-hidden">
        ...
        <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
          ...
          <span className="text-[9px] tracking-[0.15em] uppercase px-2.5 py-1 rounded-sm font-sans font-medium self-start"
  ...
  return (
    <section className="px-6 md:px-12 py-20 bg-[#F9F5F2]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-6">
        {featured.map((p, i) => <FeaturedCard key={p.id} product={p} index={i} />)}
      </div>
