...
export default function ProductModal() {
  ...
  const handleClose = () => {
    setClosing(true)
    setTimeout(() => { setClosing(false); closeModal() }, 280)
  }
  ...
  return (
    <>
      ...
      <style>{`
        ...
        @keyframes panelIn    { from { opacity:0; transform:scale(0.92) translateY(16px); filter:blur(8px) } to { opacity:1; transform:scale(1) translateY(0); filter:blur(0) } }
        @keyframes panelOut   { from { opacity:1; transform:scale(1) translateY(0); filter:blur(0) } to { opacity:0; transform:scale(0.94) translateY(10px); filter:blur(6px) } }
        @keyframes imageReveal   { from { opacity:0; transform:scale(1.06); filter:blur(6px) } to { opacity:1; transform:scale(1); filter:blur(0) } }
        @keyframes contentReveal { from { opacity:0; transform:translateX(10px); filter:blur(4px) } to { opacity:1; transform:translateX(0); filter:blur(0) } }
      `}</style>
    </>
  )
}
