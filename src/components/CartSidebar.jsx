    ...
    <>
      ...
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-cream z-[60] flex flex-col shadow-2xl transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          ...
          <button onClick={() => setIsOpen(false)} className="text-muted hover:text-dark transition-colors text-lg">✕</button>
        </div>

        {/* Items */}
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          {items.length === 0
            ? <p className="text-center text-muted text-sm mt-12 font-sans">Tu carrito está vacío.</p>
            : items.map(item => (
                <div key={item.id} className="flex gap-4 bg-white border border-border rounded-sm p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted tracking-wide uppercase font-sans mt-0.5">{item.material} · {item.subcategory}</p>
                    <div className="flex items-center justify-between mt-2.5">
                      <span className="font-sans text-sm font-medium">{formatPrice(item.price * item.qty)}</span>
                      <div className="flex items-center gap-2.5">
                        <button onClick={() => changeQty(item.id, -1)}
                          className="w-6 h-6 border border-border rounded-full flex items-center justify-center text-sm hover:bg-dark hover:text-white hover:border-dark transition-all duration-200">−</button>
                        <span className="text-sm min-w-[16px] text-center font-sans">{item.qty}</span>
                        <button onClick={() => changeQty(item.id, 1)}
                          className="w-6 h-6 border border-border rounded-full flex items-center justify-center text-sm hover:bg-dark hover:text-white hover:border-dark transition-all duration-200">+</button>
                        <button onClick={() => removeItem(item.id)}
                          className="text-muted hover:text-red-500 text-xs ml-1 transition-colors duration-200">✕</button>
                      </div>
                    </div>
                  </div>
                </div>
